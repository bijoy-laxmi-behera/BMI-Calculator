...
def BMI():
    ...
    if bmi<=18.5:
        ...
        label3.config(text="It indicates that a person is \n slightly overweight! \n a doctor may advise to loose weight!")
                      
    else:
        label2.config(text="Obes!")
        label3.config(text="Health may be at risk if they do not in lose weight!")
...
#comment to change background of scale
style =ttk.Style()
...
Height=StringVar()
Weight=StringVar()
height=Entry(root,textvariable=Height,width=5,font='arial 50',bg="#fff",fg="#000",bd=0,justify=CENTER)#allign it in center
height.place(x=35,y=160)
Height.set(get_current_value())

weight=Entry(root,textvariable=Weight,width=5,font='arial 50',bg="#fff",fg="#000",bd=0,justify=CENTER)#allign it in center
